// Screen size = 240 x 135
// Screen Name = ESP32 ST7789
// Drive Type  = ST7796
// Set board type Adadfruit Feather S3 Reverse TFT
...
String Time, Date, Icon, z_code, forecast;
...
void setup() {
  ...
  tft.setTextSize(1);  // 1 is the default, 2 is OK but lacks extended chars. 3 is the same as 1, 4 is
  clear_screen();      // Clear screen
  StartWiFi();         // Start the WiFi service
  StartTime();
  clear_screen();  // Clear screen before moving to next station display
  display_text(10, 20, String("Started WiFi"), ST77XX_GREEN, 1);
  delay(2000);
...
void loop() {
  if (millis() > previousMillis + loopDelay || Refresh) {
    ...
    Get_Data();
    clear_screen();  // Clear screen
    drawString(40, 0, Time, CENTRE, ST77XX_YELLOW, 2);
    drawString(110, 2, Date, CENTRE, ST77XX_YELLOW, 1);
    display_text(2, 21, String(Temperature, 1) + char(247) + "  " + String(Humidity, 0) + "%", ST77XX_GREEN, 2);  // char(247) is ° symbol
    display_text(2, 40, "Dew Point   = " + String(Dewpoint, 1) + char(247), ST77XX_CYAN, 1);
    display_text(2, 50, "Wind Chill  = " + String(windchill, 1) + char(247), ST77XX_CYAN, 1);
    display_text(2, 60, "Solar rad.  = " + String(solarradiation) + "W/m2", ST77XX_CYAN, 1);
    display_text(2, 70, "SolarkW Gen = " + String(solarradiation * 1500 / 135 / 1000.0, 2) + "kWh", ST77XX_CYAN, 1);
    display_text(2, 80, "Rainfall    = " + String(RainFall, 1) + "mm", ST77XX_CYAN, 1);
    display_text(2, 90, "Rain-Rate   = " + String(RainRate, 1) + "mm/hr", ST77XX_CYAN, 1);
    ...
    display_text(2, 100, "Pressure    = " + String(AirPressure, 0) + "hPa", ST77XX_CYAN, 1);
    ...
    display_text(2, 110, "(" + PressureTrend_Str + ") " + String(Trend, 1) + ", " + z_code, ST77XX_CYAN, 1);
    display_text(2, 125, forecast, ST77XX_RED, 1);
    display_text(185, 125, GetBatteryVoltage() + "v," + String(maxlipo.cellPercent(), 0) + "%", ST77XX_CYAN, 1);
    DisplayWindDirection(188, 62, winddirection, windspeed, 40, ST77XX_YELLOW, 1);
    if (WiFi.status() == WL_CONNECTED) display_text(5, 3, "Wi-Fi", ST77XX_RED, 1);
    else StartWiFi();
  }
}
...
void Get_Data() {  //client function to send/receive GET request data.
  ...
  display_text(10, 20, String("Getting Data..."), ST77XX_GREEN, 1);
...
void Decode_Response(String input) {
  ...
  Serial.println("Update Time      = " + Time + " " + Date);
...
//void arrow(int x, int y, int asize, float aangle, int pwidth, int plength, int colour) 15, 25
void DisplayWindDirection(int x, int y, float angle, float windspeed, int Cradius, int colour, int Size) {
  arrow(x, y, Cradius - (Cradius / 1.95), angle, Cradius / 3, Cradius / 2.5, colour);  // Show wind direction on outer circle of width and length
  int dxo, dyo, dxi, dyi;
  tft.drawCircle(x, y, Cradius, colour);         // Draw compass circle
  tft.drawCircle(x, y, Cradius + 1, colour);     // Draw compass circle
  tft.drawCircle(x, y, Cradius * 0.75, colour);  // Draw compass inner circle
  for (float a = 0; a < 360; a = a + 22.5) {
    dxo = Cradius * cos((a - 90) * PI / 180);
    dyo = Cradius * sin((a - 90) * PI / 180);
    if (a == 45) drawString(dxo + x + 5, dyo + y - 10, "NE", CENTRE, colour, Size);
    if (a == 135) drawString(dxo + x + 4, dyo + y + 5, "SE", CENTRE, colour, Size);
    if (a == 225) drawString(dxo + x - 14, dyo + y + 5, "SW", CENTRE, colour, Size);
    if (a == 315) drawString(dxo + x - 12, dyo + y - 10, "NW", CENTRE, colour, Size);
    dxi = dxo * 0.9;
    dyi = dyo * 0.9;
    tft.drawLine(dxo + x, dyo + y, dxi + x, dyi + y, colour);
    dxo = dxo * 0.75;
    dyo = dyo * 0.75;
    dxi = dxo * 0.9;
    dyi = dyo * 0.9;
  ...
  drawString(x - 0, y - (Cradius + 12), "N", CENTRE, colour, Size);
  drawString(x - 0, y + Cradius + 5, "S", CENTRE, colour, Size);
  drawString(x - Cradius - 10, y - 4, "W", CENTRE, colour, Size);
  drawString(x + Cradius + 5, y - 4, "E", CENTRE, colour, Size);
  drawString(x - 6, y - 15, WindDegToOrdDirection(angle), CENTRE, colour, Size);
  drawString(x - 15, y - 4, String(windspeed, 0) + "mph", CENTRE, colour, Size);
  drawString(x - 9, y + 10, String(angle, 0) + char(247), CENTRE, colour, Size);
}
...
//#########################################################################################
void arrow(int x, int y, int asize, float aangle, int pwidth, int plength, int colour) {
  ...
  tft.fillTriangle(xx1, yy1, xx3, yy3, xx2, yy2, colour);
}
...
void StartWiFi() {
  display_text(10, 20, String("Starting WiFi..."), ST77XX_GREEN, 1);
  WiFi.disconnect();
...
String ZCode(String msg) {
  ...
  int icon_y_size = 80;
  String message = "";
  // void drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t colour);
  if (msg == "A") {
    //Icon = "https://openweathermap.org/img/wn/01d@2x.png";
    //tft.drawBitmap(x_pos, y_pos, Img_01d_2x, icon_x_size, icon_y_size, ST77XX_WHITE);
    message = "Settled fine weather";
  }
  if (msg == "B") {
    //Icon = "https://openweathermap.org/img/wn/01d@2x.png";
    //tft.drawBitmap(x_pos, y_pos, icon_x_size, icon_y_size, Img_01d_2x);
    message = "Fine weather";
  }
  if (msg == "C") {
    //Icon = "https://openweathermap.org/img/wn/02d@2x.png";
    //tft.drawBitmap(x_pos, y_pos, icon_x_size, icon_y_size, Img_02d_2x);
  ...
  if (msg == "E") {
    ...
    //tft.drawBitmap(x_pos, y_pos, icon_x_size, icon_y_size, Img_10d_2x);
  ...
  if (msg == "J") {
    //Icon = "https://openweathermap.org/img/wn/10d@2x.png";
    //tft.pushImag(x_pos, y_pos, icon_x_size, icon_y_size, Img_02d_2x);
  ...
  if (msg == "L") {
    ...
    //tft.drawBitmap(x_pos, y_pos, icon_x_size, icon_y_size, Img_04d_2x);
  ...
  if (msg == "O") {
    ...
    //tft.drawBitmap(x_pos, y_pos, icon_x_size, icon_y_size, Img_09d_2x);
